...
import { getEncodedXPortalLoginSchemaUrl, getXPortalSchemaUrl } from '../services/wallet/xportalDeeplink';
 ...
 */
export const openXPortalForLogin = connectorUri => {
  try {
    const encodedSchemaUrl = getEncodedXPortalLoginSchemaUrl(connectorUri);
    Linking.canOpenURL(encodedSchemaUrl).then(supported => {
